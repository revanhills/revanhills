...
export default function BookPage() {
  const [stay, setStay] = useState<"villa" | "room">("room");
  ...
  useEffect(() => {
    ...
    const requestedStay = params.get("stay") === "villa" ? "villa" : "room";
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const message = [
      "*NEW BOOKING ENQUIRY — REVAN HILLS*",
      "",
      "*BOOKING & PROPERTY DETAILS*",
      `Property: ${stay === "villa" ? "Entire Revan Hills villa" : selectedRoom}`,
      `Stay dates: ${checkIn} to ${checkOut}`,
      `Duration: ${nights} night${nights === 1 ? "" : "s"}`,
      `Guests: ${guests}`,
      `Meal preference: ${data.get("meals")}`,
      "",
      "*GUEST DETAILS*",
      `Name: ${data.get("name")}`,
  ...
  return <main className="booking-page-new"><SiteHeader /><div className="booking-back-strip"><div className="shell"><Link className="back-link" href="/"><ArrowLeft size={17} /> Back to property</Link></div></div><div className="shell booking-page-grid"><section><p className="eyebrow">REQUEST TO BOOK</p><h1>Let’s find your stay.</h1><p className="booking-intro">Choose your stay, dates and guests. We’ll prepare the request for Devang on WhatsApp.</p>
    <form className="booking-form" onSubmit={submit}><fieldset><legend>1. How would you like to stay?</legend><div className="booking-option-grid"><button type="button" className={stay === "room" ? "active" : ""} onClick={() => {setStay("room"); setGuests(Math.min(guests, 3));}}><strong>Private room</strong><span>Choose one of four rooms</span><i>{stay === "room" && <Check />}</i></button><button type="button" className={stay === "villa" ? "active" : ""} onClick={() => {setStay("villa"); setGuests(Math.min(guests, 8));}}><strong>Entire villa</strong><span>4 bedrooms · up to 8 guests</span><i>{stay === "villa" && <Check />}</i></button></div>{stay === "room" && <div className="room-choice"><div className="room-choice__heading"><div><span>Preferred room</span><strong>Choose your room</strong></div><small>Tap a room to select it and view its tour</small></div><div className="room-thumbnail-grid" aria-label="Choose a preferred room">{roomItems.map((item) => { const selected = room === item.slug; return <button key={item.slug} type="button" aria-pressed={selected} className={`room-thumbnail-option ${selected ? "active" : ""}`} onClick={() => openRoomTour(item.slug)}><span className="room-thumbnail-option__image"><Image src={item.image} alt={item.name} fill sizes="(max-width: 760px) 42vw, 220px" />{selected && <i><Check /></i>}</span><span className="room-thumbnail-option__copy"><strong>{item.name}</strong><small>{item.tone}</small><em>{selected ? "Selected room" : "View photos & videos"}</em></span></button>; })}</div></div>}</fieldset>
